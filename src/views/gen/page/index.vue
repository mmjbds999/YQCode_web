	...
	<el-container>
		<el-header>
			...
			<div class="right-panel">
				<div class="right-panel-search">
					<el-input v-model="search.keyword" placeholder="项目名称" clearable></el-input>
		...
		<el-main class="nopadding">
			<scTable ref="table" :apiObj="apiObj" row-key="id" @selection-change="selectionChange" @dataChange="onDataLoaded" stripe highlightCurrentRow>
				...
				<el-table-column label="页面名称" prop="name" width="250"></el-table-column>
				<el-table-column label="英文名" prop="enName" width="150"></el-table-column>
				<el-table-column label="页面类型" prop="type" width="250"></el-table-column>
				<el-table-column label="简介" prop="vuePort" width="250"></el-table-column>
...
export default {
	...
	data() {
		return {
			...
			apiObj: this.$API.system.pagesAPI.list,
			selection: [],
			search: {
				keyword: null
			},
	...
	methods: {
		...
		async table_del(row){
			...
			let res = await this.$API.system.pagesAPI.del.delete(reqData);
			if(res.code === 200){
				this.$refs.table.refresh()
		...
		async batch_del(){
			this.$confirm(`确定删除选中的 ${this.selection.length} 项吗？`, '提示', {
				...
				this.$API.system.pagesAPI.delBatch.delete(ids).then((res) => {
					if(res.code === 200){
						this.$nextTick(() => {
							this.$refs.table.refresh();
